...
// {}, }, { status: 200 } ); } catch (error) { console.error("Error:", error);
// return json( { success: false, data: {}, error: { msg: "An error occurred
// while adding item to folder" }, }, { status: 500 } ); } }
// ---------------------- createReview ------------------------------ import {
// LoaderFunctionArgs, json } from "@remix-run/node"; //import { createReview }
// from "../../../lib/database/review"; import { createRating } from
// "../../../lib/database/rate"; import { handleRating } from
// "../../../lib/dataRetrieve/handleRating"; export async function loader({
// request }: LoaderFunctionArgs) { const userId: number = 2; // Provide a
// valid user ID for testing const itemId: number = 43; // Provide the ID of
// the existing item (movie, song, or book) const reviewContent: string =
// "Worst song that i have heard so far."; // Provide the review content const
// rating: number = 4; // Provide the rating (out of 5) to be assigned try { //
// Create a new review //const newReview = await createReview(userId, itemId,
// reviewContent); const newReview = await handleRating(userId, itemId,
// rating); console.log("Created Review:", newReview); return json( { success:
// true, data: newReview, error: {}, }, { status: 200 } ); } catch (error) {
// console.error("Error:", error); return json( { success: false, data: {},
