  ...
  try {
    ...
    for (const recommendation of recommendations) {
      if (
        ...
        mergedRecommendations.books = [
          ...mergedRecommendations.books,
          ...recommendation.books,
        ];
        mergedRecommendations.movies = [
          ...mergedRecommendations.movies,
          ...recommendation.movies,
        ];
        mergedRecommendations.songs = [
          ...mergedRecommendations.songs,
...
export async function fetchRecommendationsWithDetails(
  userId: number,
): Promise<any> {
  try {
    ...
    const booksPromises = cleanRecommendations.books.map((book) =>
      handleBookSearchAPI(book),
    );
    const moviesPromises = cleanRecommendations.movies.map((movie) =>
      handleMovieSearchAPI(movie),
    );
    const songsPromises = cleanRecommendations.songs.map((song) =>
      handleSongSearchAPI(song),
    );
